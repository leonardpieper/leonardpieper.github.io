...
//     // $("#d" + outputElementId).find(".android-kursList").sort(function(a, b) {
//     //     a.dataset.timestamp = a.dataset.timestamp * -1;
//     //     b.dataset.timestamp = b.datase var cache = cacheKurse.returnCache();
//     output = "";
...
var kurse = {
    getKurse: function (fn) {
        var cachedTime = kursCache.getCacheTime();
        var currTime = new Date().getTime();

        if (cachedTime === -1 || cachedTime + 604800000 < currTime) {
            if (mAuth.currentUser != null) {
                this.getKurseOnline(fn);
            }
        } else {
            ...
            for (var kurs in cacheKurse) {
                ...
                if (cacheKurse[kurs].type === "online") {
                    output += "<li class='mdl-list__item android-kursList' data-timestamp='" + timestamp + "' data-kurs='" + name + "'><img class='kursIcon' alt='" + name + " Icon' src='" + getIcon(name) + "'><a id='" + kursID + "Id' class='kursLink' href=\"javascript:setKurs(\'" + name + "\')\">" + name + "</a><a class='kurs-close-icon' href=\"javascript:leaveKursDialog(\'" + kursID + "\')\"><i class='material-icons'>&#xE879;</i></a><a class='kurs-rem-icon operatorArea' href=\"javascript:removeKursDialog(\'" + kursID + "\')\"><i class='material-icons'>&#xE92B;</i></a></li>";
                } else {
                    output += "<li class='mdl-list__item android-kursList' data-kurs='" + name + "'><img class='kursIcon' alt='" + name + " Icon' src='" + getIcon(name) + "'><p id='" + kursID + "Id' class='kursLink' >" + name + "</p><a class='kurs-close-icon' href=\"javascript:leaveKursDialog(\'" + kursID + "\')\"><i class='material-icons'>&#xE879;</i></a></li>";
                }
            }
            fn(output);
        }
        ...
        firebase.database().ref(ref).on('value', function (snapshot) {
            ...
            if (snapshot.val() === null) {
                ...
                snapshot.forEach(function (childSnapshot) {
                    ...
                    var kursID = title.replace(/ /g, "__");

                    if (type === "online") {
                        output += "<li class='mdl-list__item android-kursList' data-kurs='" + title + "'><img class='kursIcon' alt='" + title + " Icon' src='" + getIcon(title) + "'><a id='" + kursID + "Id' class='kursLink' href=\"javascript:setKurs(\'" + title + "\')\">" + title + "</a><a class='kurs-close-icon' href=\"javascript:leaveKursDialog(\'" + kursID + "\')\"><i class='material-icons'>&#xE879;</i></a><a class='kurs-rem-icon operatorArea' href=\"javascript:removeKursDialog(\'" + kursID + "\')\"><i class='material-icons'>&#xE92B;</i></a></li>";
                    } else {
                        output += "<li class='mdl-list__item android-kursList' data-kurs='" + title + "'><img class='kursIcon' alt='" + title + " Icon' src='" + getIcon(title) + "'><p id='" + kursID + "Id' class='kursLink' >" + title + "</p><a class='kurs-close-icon' href=\"javascript:leaveKursDialog(\'" + kursID + "\')\"><i class='material-icons'>&#xE879;</i></a></li>";
                    }
...
var kursCache = {
    /**
     * Setzt den Cache auf den ursprünglichen Wert zurück
     */
    newCache: function () {
        localStorage.setItem("kurse", undefined);
    },
    ...
    /**
     * Fürgt einen neuen Kurs hinzu
     * @param {string} kursName Der Name des Kurses, der hinzugefügt werden soll
     * @param {string} kursType Der Kurstyp: "online" oder "offline"
     */
    addCache: function (kursName, kursType) {
        var d = new Date();
        var currTimeMill = d.getTime();
        var oldCache = localStorage.getItem("kurse");
    ...
    /**
     * Gibt den gesammten Cache zurück
     * @return {JSON} Der zurückgegebene Cache
     */
    getCache: function () {
        var cache = localStorage.getItem("kurse");
    ...
    /**
     * Gibt die Zeit in Millisekunden zurück, die gecached wurde
     * @return {Number} Die zurückgegebene Zeit in Millisekunden
     */
